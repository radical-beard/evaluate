...
| `system-scene.evt` | system `.evt` (scene-scoped) | `scenes:` scoping, `on_enter`/`on_exit`/`on_update`, `scene.current()` / `scene.change(name, ctx)` (the context the destination reads via `scene.context()`). |
| `player.behavior.evt` | behavior `*.behavior.evt` | `self`-bound behavior, frontmatter `properties:`, `config.player.*`, polling `actions.Gameplay.Move.vector` in `on_physics_update`, reading a sibling behavior's attribute, the read-mutate-assign-back struct pattern. |
| `sprint.behavior.evt` | behavior (GAS-lite) | `attributes:` (stamina with regen/exhaust/recover + a speed pool), `abilities:`, action `subscribe` (press/release, in `on_load`) driving a channel, `on_ended`. |
...
| `scene-global.scene` | `global.scene` manifest | persistent nodes, `start_scene`, `controls = "controls.toml"`, a `behaviors = [...]` list (two behaviors, order matters), child nesting. |
| `controls.toml` | controls map (TOML) | physical bindings → actions per scenario: a stick + WASD keys feeding one `Move` vector, multi-bound `Sprint`, an analog axis, the reserved `[Always]` and `[settings]` sections. |
| `scene-swappable.scene` | `*.scene` | a swappable tree: the scene-level `scenario` key, `behaviors` with per-attachment `params` (incl. a hand-authored dna hash), `machines`, an inline `_type` resource. |
